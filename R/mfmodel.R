...
.mfmodel.Schechter = function(x = NULL, p = NULL, sigma = NULL, output = 'density') {
  ...
  if (output == 'density') {
    ...
    return('phi(x) = dN/(dVdx) = log(10)*phi_star*(10^x/M_star)^(alpha+1)*exp(-10^x/M_star), where p = (log10(phi_star),log10(M_star),alpha)')
...
.mfmodel.PL = function(x = NULL, p = NULL, sigma = NULL, output = 'density') {
  ...
  if (output == 'density') {
    ...
    return('phi(x) = dN/(dVdx) = scale*(10^x)^alpha\n, where p = (log10(scale),alpha)')
  } else if (output == 'parameters') {
    if (is.null(sigma)) {
      cat(sprintf('log10(scale) = %9.2f\n',p[1]))
      cat(sprintf('alpha        = %9.2f\n',p[2]))
    } else if (is.array(sigma)) {
      cat(sprintf('log10(scale) = %9.2f (+%3.2f -%3.2f)\n',p[1],sigma[1,1],sigma[2,1]))
      cat(sprintf('alpha        = %9.2f (+%3.2f -%3.2f)\n',p[2],sigma[1,2],sigma[2,2]))
    } else {
      cat(sprintf('log10(scale) = %9.2f (+-%3.2f)\n',p[1],sigma[1]))
      cat(sprintf('alpha        = %9.2f (+-%3.2f)\n',p[2],sigma[2]))
    }
  } else {
    stop('Unknown output.')
