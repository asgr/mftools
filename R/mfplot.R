...
mfplot <- function(mf,
  ...
  if (is.null(ylim)) {
    ylim = c(1e-3*max(mf$fit$plot$phi),2*max(max(mf$fit$plot$phi),bin$phi))
  }
  
  # make polygons
  show.polygons = length(mf$fit$plot$phi.uncertainty.05)>0
  if (show.polygons) {
    poly.x = 10^c(mf$fit$plot$x,rev(mf$fit$plot$x))
    poly.y.50 = pmax(ylim[1],c(mf$fit$plot$phi.uncertainty.25,rev(mf$fit$plot$phi.uncertainty.75)))
    poly.y.90 = pmax(ylim[1],c(mf$fit$plot$phi.uncertainty.05,rev(mf$fit$plot$phi.uncertainty.95)))
  }
  ...
  # plot central fit
  lines(10^mf$fit$plot$x[mf$fit$plot$phi>0],mf$fit$plot$phi[mf$fit$plot$phi>0],col='red',lwd=1.5)
